...
let awardbtns = d3.selectAll(
  'input[name="bestOf"],input[name="sortDirection"]'
);
awardbtns.on("input", (e) => {
  let sortLow =
    document.querySelector("input[name='sortDirection']:checked").value ==
    "ascending";
...
function sort(by = catData, isReversed = false) {
  ...
  sortedHighArray.forEach(function (d, i) {
    ...
    let bakeryChart = d3
      .selectAll(".bakery-restOf")
      .filter((dd, ii) => ii == i);

    if (i !== 0) {
      let colors = ["#502A27", "#E86663", "#EDB834", "#923F3D", "#99C983"];

      let labelsDiv = bakeryChart
        .select(".chart-restOf")
        .append("div")
        .classed("labels", true);
      let scoreBarDivs = bakeryChart
        .select(".chart-restOf")
        .append("div")
      ...
      for (let categorie in categories) {
        let rowsLabels = bakeryChart
          .selectAll(".labels")
          .append("div")
          .classed("catLabel-result", true)
          .text(categorie);

        let scoreBars = bakeryChart
          ...
          .style(
            "background",
            (dd, ii) =>
              `linear-gradient(to right, currentcolor 0%, currentcolor ${
                categories[categorie] * 20
              }%, transparent ${categories[categorie] * 20}%, transparent 100%`
          );
            ...
            (enter) =>
              enter
                ...
                .style(
                  ...
                  (category, i) =>
                    `linear-gradient(to right, currentcolor 0%, currentcolor ${
                      Math.max(score - i, 0) * 100
                    }%, transparent  ${
                      Math.min(score - i, 1) * 100
                    }%, transparent 100%`
                ),
            (update) => update,
            (exit) => exit.remove()
